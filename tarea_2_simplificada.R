...
covars <- intersect(c("gender", "age", "religion", "caste", "education", "homeBuilt"), names(df))
controls_simple <- intersect(c("age", "birthplace", "gender", "caste"), names(df))
controls <- intersect(c("gender", "birthplace", "age", "religion", "caste", "education", "homeBuilt"), names(df))
...
model_balance <- lm(reformulate(covars, response = "treat"), data = df)
...
mod_inter <- lm(total_expenditure ~ 0 + estrato + estrato:treat, data = df_strat)
summary(mod_inter)
...
double_lasso_fe <- function(yname, data = df, xvars = controls, dname = "treat", fe = "Pole") {
  ...
  sel_y <- which(as.vector(coef(cv.glmnet(X, y, alpha = 1), s = "lambda.min"))[-1] != 0)
  ...
  y_res <- if (is.null(X_sel)) y else resid(lm(y ~ X_sel))
